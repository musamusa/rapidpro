    ...
    class Broadcast(ModalMixin, OrgObjPermsMixin, SmartUpdateView):
        ...
        class BroadcastForm(forms.ModelForm):
            def __init__(self, *args, **kwargs):
                ...
                super().__init__(*args, **kwargs)
                FlowCRUDL.Broadcast.flow_params_fields = []
                FlowCRUDL.Broadcast.flow_params_values = []
                ...
                for counter, flow_param in enumerate(sorted(self.flow.get_trigger_params())):
                    ...
                    if f"flow_param_field_{counter}" not in FlowCRUDL.Broadcast.flow_params_fields:
                        FlowCRUDL.Broadcast.flow_params_fields.append(f"flow_param_field_{counter}")
                    if f"flow_param_value_{counter}" not in FlowCRUDL.Broadcast.flow_params_values:
                        FlowCRUDL.Broadcast.flow_params_values.append(f"flow_param_value_{counter}")
